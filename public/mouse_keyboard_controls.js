let detectedDims = [];
let lockedOnNode = false;
let lockedNode = NaN;

function mousePressed() {
  if ((lockedNode = findNodeByCoord(mouseX, mouseY))) {
    lockedOnNode = true;
  } else {
    lockedOnNode = false;
  }
...
function doubleClicked() {
  ...
  if (!curr) return;
  if (dataInput.value()) curr.data = dataInput.value();
}
...
function keyPressed() {
  ...
  if (keyCode === SHIFT) {
    ...
    if (
      detectedDims.length >= 4 &&
      findNodeByCoord(detectedDims[0], detectedDims[1]) && //this is just spaghetti
      findNodeByCoord(detectedDims[2], detectedDims[3])
    ) {
      let fromIdx = nodeIdxFinder(
        nodes,
        findNodeByCoord(detectedDims[0], detectedDims[1]).data
      );
      let toIdx = nodeIdxFinder(
        nodes,
        findNodeByCoord(detectedDims[2], detectedDims[3]).data
      );
    ...
    } else if (detectedDims.length >= 4) {
      //the second click is missed
      for (let i = 0; i < 4; i++) detectedDims.splice(2, 2);
    }
